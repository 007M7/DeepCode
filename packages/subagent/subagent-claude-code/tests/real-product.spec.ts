...
describe('real Claude Agent SDK 0.3.221 and its distributed Claude Code 2.1.221 fixture', {
  timeout: 60_000,
}, () => {
  it('inherits host settings and sends the exact task and fake key to local Messages', async () => {
    const sentinel = 'REAL_CLAUDE_CODE_SENTINEL_2_1_221'
    const task = 'Return the fixture sentinel exactly.'
    const { harness, fixture } = await realHarness({
      kind: 'complete',
      text: sentinel,
    })
    expect(sdkPackage.version).toBe('0.3.221')
    expect(sdkPackage.claudeCodeVersion).toBe('2.1.221')
    expect(sdkPackage.optionalDependencies[platformPackage]).toBe('0.3.221')
    const version = await execFileAsync(process.platform === 'win32' ? claudeBin : harness.executable, ['--version'], {
      env: { ...process.env, ...harness.env },
    })
    expect(version.stdout.trim()).toBe('2.1.221 (Claude Code)')
    ...
    expect(initMessage?.claude_code_version).toBe('2.1.221')
    if (process.platform === 'win32') {
