...
The TTY welcome area is a bordered blue pixel-whale card; transcript and successful-tool output below it stay compact.

User and assistant messages use compact `YOU` and `DEEPCODE` labels without per-message borders. The mutable streaming area shows only the latest twelve lines so a long response cannot leave repaint residue in Windows CMD; the complete response enters scrollback once at turn end. Headings, bullets, fenced code, and bold spans receive lightweight terminal formatting. Tool calls stay in the working state while a turn runs, then commit as one human-readable count by operation; failed calls remain separate, and Ctrl+O reveals the latest summary's raw arguments.

While work is active, the status row shows the foreground task's elapsed time and the age of its latest Session event. Background subagents remain visible independently with their running count and oldest elapsed time, so a quiet long-running task can be distinguished from an idle CLI.

The live usage row reports the complete owned task tree's cache-hit rate, billed and uncached input, and output. Auxiliary model calls and descendant sessions must enter the same durable usage ledger before they appear here; the CLI does not infer billing from visible transcript rows. Monetary cost displays `n/a` until a route-aware price source supplies an estimate.
...
Start it with `deepseek`, `dsh cli`, or `dsh --profile cli`. A TTY receives the compact `DeepCode` Ink interface without alternate-screen ownership, with durable terminal scrollback, streamed answers, one row per successful tool call, a live identity/status line, selectable approvals and questions, command completion, input history, and a masked credential prompt. Committed output uses Ink's immutable `Static` region; streaming changes only the bottom live response and composer, which keeps the host terminal scrollbar stable. Ctrl+O opens only the latest tool details in that live region without rewriting or unboundedly mirroring scrollback. `/clear` replaces the Ink owner and clears both the visible screen and retained terminal scrollback before drawing the current identity. Redirected stdin/stdout use deterministic plain text without ANSI escapes. One surface owns stdin for chat, credentials, approvals, and model questions. Complete chat messages submitted while the Agent is running enter an in-memory FIFO and dispatch after the current turn becomes idle; approvals and model questions retain the shared composer when they arrive. An active Ctrl+C cancels work, while idle Ctrl+C or EOF performs a bounded shutdown.
