...
Start it with `deepseek`, `dsh cli`, or `dsh --profile cli`. A TTY receives the `DeepCode` Ink interface without alternate-screen ownership, with the product preview's looped, open-mouth blue pixel whale and white eye, durable terminal scrollback, streamed answers, tool activity, a live identity/status line, selectable approvals and questions, command completion, input history, and a masked credential prompt. The whale uses ANSI background pixels, so native Windows CMD renders its silhouette without Unicode block glyphs. Committed output uses Ink's immutable `Static` region; streaming changes only the bottom live response and composer, which keeps the host terminal scrollbar stable. Ctrl+O opens only the latest tool details in that live region without rewriting or unboundedly mirroring scrollback. `/clear` replaces the Ink owner and clears both the visible screen and retained terminal scrollback before drawing the current identity. Redirected stdin/stdout use deterministic plain text without ANSI escapes. One surface owns stdin for chat, credentials, approvals, and model questions; an active Ctrl+C cancels work, while idle Ctrl+C or EOF performs a bounded shutdown.
