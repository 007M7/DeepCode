...
使用 `deepseek`、`dsh cli` 或 `dsh --profile cli` 启动。TTY 会得到基于 Ink、不会占用 alternate screen 的终端界面，其中包含蓝色像素鲸鱼、可保留的终端回滚、流式回答、工具活动、实时状态行、可选择的审批与问题、命令补全、输入历史和遮罩凭据提示。重定向的 stdin/stdout 使用不含 ANSI 转义的确定性纯文本。一个 surface 统一持有聊天、凭据、审批和模型问题所用的 stdin；运行中的 Ctrl+C 取消工作，空闲 Ctrl+C 或 EOF 执行有界退出。
