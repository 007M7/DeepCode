...
`usageLedger` carries the same buckets for billing displays and adds every durable `llm/aux-usage` record. Main-request samples retain `tokenUsage` deduplication, while Web Search, title generation, and other auxiliary requests each contribute independently.
