...
This reference defines the `deepseek` terminal entry plus the profile, local-CLI alias, web-alias, plugin-management, and config-dump command modes. `deepseek` selects the shipped `cli` profile; `dsh` argv is parsed once through [`src/args.ts`](../src/args.ts), and both bins share [`src/launcher.ts`](../src/launcher.ts) for dispatch.
...
Launcher flags must come before app arguments, and the launcher's parser consumes one `--`: an app argument that must arrive as a literal `--` needs `-- --`. A first app argument equal to `cli`, `web`, or `plugin` selects that subcommand instead. `ctx.cmdlineArgs.get()` is a shared immutable read: multiple plugins may parse the same snapshot, while a profile with no reader ignores its app arguments.

The shipped apps own these command lines:

| Profile | Arguments |
|---|---|
| `cli` | no positional arguments; `--help` prints the local CLI help |
| `web` | `--host`, `--port`, repeatable `--trusted-host` |
| `headless` | the task text, as the positional argument |

The local interactive profile (`deepseek`, `dsh cli`, or `dsh --profile cli`) creates one persisted Agent for the invoking directory and reuses it across turns. A TTY uses one non-full-screen Ink tree for the welcome card, transcript, streamed response, tool state, status, composer, approvals, questions, and masked secrets. Non-TTY streams use a queued plain-text reader with no ANSI escapes. Committed `assistant/chunk` text deltas stream immediately; an `assistant/message` supplies only a step that emitted no text delta. The runner waits for each turn to become idle and flushes the Session before accepting the next input.

Built-ins are `/login`, `/logout`, `/status`, `/model`, `/models`, `/new`, `/sessions`, `/resume`, `/clear`, `/help`, and `/exit`. Authentication goes through the selected provider's settings-owned credential reference and `ctx.credentials`, never through argv or Session events. Model selection resolves the provider, exact model id, and optional reasoning effort through `ctx.llm`, then updates the current Agent and saves the default when possible. Session listing and resume are limited to root sessions in the invoking workspace; sessions with an Agent preset are refused because this profile does not reproduce that composition. Any other slash command dispatches through `ctx.commands`, which includes `/compact` and `/goal` in the shipped tree.

Enter sends; Ctrl+J or Shift+Enter inserts a newline; Up/Down navigates history or a picker; Tab completes one matching slash command; Ctrl+A/E/U/K/W provide line editing; Ctrl+L clears; Ctrl+O toggles tool details; Ctrl+N starts a new session; and Escape/Ctrl+C cancel active work. Idle Ctrl+C and empty Ctrl+D exit. The Ink tree does not enter the alternate screen, so completed output remains in terminal scrollback.

Ctrl+C while the Agent or a cancellable plugin command is running cancels only that operation and returns to the composer after settlement. Ctrl+C while idle, or stdin EOF, closes the surface; shutdown unregisters its answerers and event listener, waits for idleness, flushes the Session, disposes the owned Agent handle, and then requests a successful launcher exit. The base bundle supplies filesystem policy and tools, approval policy, permission presets, and the platform shell. On POSIX the CLI bundle mounts the persistent terminal Service Definition, bash provider, and terminal-tool Consumer together. Windows disables that complete three-plugin seam and keeps the base one-shot PowerShell tool; this release does not claim ConPTY support.
