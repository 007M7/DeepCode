...
The repository-root `install-deepcode.cmd` is the supported downloaded-source installer on Windows. It restores the locked workspace, builds Host artifacts, globally links this package, and persists npm's global command directory on the user's `PATH`. It removes the generated `deepseek.ps1` so PowerShell resolves the same `deepseek.cmd` as CMD even under Restricted script policy, then verifies both shells by command name. Re-running it updates the link and build in place. The source checkout must remain at its installed path; `uninstall-deepcode.cmd` removes the linked package but leaves the shared npm command directory on `PATH` for other global tools.
