...
/**
 * Apply the terminal product title for the shipped interactive profile.
 * @param profile - Profile selected by either the `deepseek` or `dsh` entry.
 */
export function applyProfileProcessIdentity(profile: string): void {
  if (profile === 'cli') process.title = 'DeepCode'
}
...
export async function runInvocation(invocation: DshInvocation): Promise<void> {
  switch (invocation.mode) {
    case 'profile': {
      applyProfileProcessIdentity(invocation.profile)
