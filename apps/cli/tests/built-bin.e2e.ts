...
const deepseekBin = join(repoRoot, 'apps/cli/lib/deepseek.js')
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('ships the direct deepseek bin beside dsh and reads the same version', async () => {
    expect(cliManifest.bin).toMatchObject({ dsh: 'lib/bin.js', deepseek: 'lib/deepseek.js' })
    const result = await execa(process.execPath, [deepseekBin, '--version'], { reject: false })
    expect({ code: result.exitCode, stdout: result.stdout, stderr: result.stderr })
      .toEqual({ code: 0, stdout: cliVersion, stderr: '' })
  })
